...
parser.add_argument('--early_stopping', default=41 , type=int, help='')
...
def main():
    ...
    model.add(Bidirectional(CuDNNLSTM(args.lstm2, return_sequences=True)))
    ...
    # 模型评估
    with CustomObjectScope({'Attention_layer': Attention_layer}):
        model = keras.models.load_model(best_model_path) #直接load_model识别不了Attention_layer
    ...
    print("*****最后一个epoch的模型表现***************************")
    print("Train rmse of the last model: {}".format(np.sqrt(history.history['loss'][-1])))
    print("Validation rmse the last model: {}".format(np.sqrt(history.history['val_loss'][-1])))
    print("Test rmse of the last model: {}".format(rmse))
    print("*****最佳模型表现***************************")
